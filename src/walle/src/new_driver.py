...
class Driver:
    def __init__(self, position_source, threshold=0.1):
        ...
        self._lidar_sub = rospy.Subscriber(
            "base_scan", LaserScan, self._lidar_callback, queue_size=10
        )
        ...
        self._action_server = actionlib.SimpleActionServer(
            "nav_target",
            NavTargetAction,
            execute_cb=self._action_callback,
            auto_start=False,
        )
    ...
    def _action_callback(self, goal):
        rospy.loginfo(
            f"Got an action request for ({goal.goal.point.x:.2f}, {goal.goal.point.y:.2f})"
        )
    ...
    def _lidar_callback(self, lidar):
        if self._target_point:
            ...
            try:
                target = self.transform_listener.transformPoint(
                    "base_link", self._target_point
                )
                x = target.point.x
                y = target.point.y
                ...
                if self.close_enough_to_waypoint(
                    distance, (target.point.x, target.point.y), lidar
                ):
                    self._target_point = None
